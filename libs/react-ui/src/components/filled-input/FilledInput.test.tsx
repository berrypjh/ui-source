...
describe('<FilledInput />', () => {
  ...
  describe('rendering', () => {
    it('기본적으로 root와 native input을 렌더링해야 한다', () => {
      ...
      expect(input).toHaveClass(filledInputClasses.input);
    ...
    it('root props를 InputBase root에 전달해야 한다', () => {
      render(<FilledInput data-testid="root" data-test="test" />);
  ...
  describe('with FormControl', () => {
    ...
    it('FormControl 상태는 props로 override할 수 있어야 한다', () => {
      const FilledInputInForm = (props: ComponentProps<typeof FilledInput>) => (
        <FormControl error size="md" hiddenLabel>
          <FilledInput data-testid="root" {...props} />
        </FormControl>
      );

      const { setProps } = render(<FilledInputInForm />);
  ...
  describe('input class composition', () => {
    it('native input에 filled input 전용 클래스와 custom class를 함께 유지해야 한다', () => {
      render(
